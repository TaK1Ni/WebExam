...
function fillRouteByCollection(data) {
  ...
  for (var i = 0; i < displayedData.length; i++) { 
    ...
    var mainObjectItems = item['mainObject'].split(/[-,–](?=.)/); //используем регулярку 
    ...
    }).bind(null, item.id));  //привязка значения ID элемента к функции обработчика события клика кнопки.
